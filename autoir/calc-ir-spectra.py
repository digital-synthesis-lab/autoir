...
T = 300.0  # K
zoom_wavenum = 4000.0  # cm^-1
fraction_autocorrelation_function_to_fft = (
    0.01  # since you only want to fft the part that has meaningful statistics,
)
...
if autocorrelation_option == 1:
    ...
    autocorr_x_full = signal.fftconvolve(dipole_x_shifted, dipole_x[::-1], mode="same")[
        (-len(time)) :
    ] / np.arange(len(time), 0, -1)
    autocorr_y_full = signal.fftconvolve(dipole_y_shifted, dipole_y[::-1], mode="same")[
        (-len(time)) :
    ] / np.arange(len(time), 0, -1)
    autocorr_z_full = signal.fftconvolve(dipole_z_shifted, dipole_z[::-1], mode="same")[
        (-len(time)) :
    ] / np.arange(len(time), 0, -1)
    autocorr_full = autocorr_x_full + autocorr_y_full + autocorr_z_full
    # Truncate the autocorrelation array
    autocorr = autocorr_full[
        : int(len(time) * fraction_autocorrelation_function_to_fft)
    ]
...
timestep = (time[1] - time[0]) * 1.0e-15  # converts time from femtoseconds to seconds
